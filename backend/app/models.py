...
class Insumo(Base):
    """Un insumo o repuesto del stock. Tiene compras (entradas), consumos
    (salidas) y ajustes manuales (correcciones)."""
    ...
    ajustes: Mapped[list["AjusteInventario"]] = relationship(back_populates="insumo")
...
class AjusteInventario(Base):
    """Ajuste manual de stock: corrige el conteo de un insumo sin que sea ni
    una compra (no llegó un pedido) ni un consumo (no se usó en una OT).
    Sirve para mermas, roturas, conteos físicos que no coinciden con el
    sistema, o stock encontrado que no estaba registrado.
    """
    __tablename__ = "ajustes_inventario"

    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    insumo_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("insumos.id"), nullable=False)
    tipo: Mapped[str] = mapped_column(String, nullable=False)   # "entrada" / "salida"
    cantidad: Mapped[int] = mapped_column(Integer, nullable=False)
    motivo: Mapped[str] = mapped_column(Text, nullable=False)
    fecha: Mapped[datetime | None] = mapped_column(DateTime, default=datetime.utcnow)
    registrado_por: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), nullable=True)

    # ─── Relaciones ───
    insumo: Mapped["Insumo"] = relationship(back_populates="ajustes")
...
class SolicitudServicio(Base):
    ...
    # ubicacion: dónde está el equipo/la cosa (ej: "Quirófano 3", "Piso 2 - Box 5").
    # La pide siempre el solicitante, sea equipo médico o no.
